...
var corn_plant_scene = preload("res://Croptails_scenes/objects/Plants/corn.tscn")
var tomatoe_plant_scene = preload("res://Croptails_scenes/objects/Plants/tomatoe.tscn")
...
func add_crop() -> void:
	if distance < 20.0:
		...
		if ToolManager.selected_tool == DataTypes.Tools.PlantTomato:
			var tomatoe_instance = tomatoe_plant_scene.instantiate() as Node2D
			tomatoe_instance.global_position = local_cell_position
			get_parent().find_child("CropFields").add_child(tomatoe_instance)
